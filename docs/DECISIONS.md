...
**Aceptada — Opción B.** Cada grupo de seguridad se resume con un estado principal determinista exclusivamente de presentación, conservando siempre todas las observaciones, ingredientes, fuentes y evidencia que lo originan. La precedencia es `PRESENCIA_IDENTIFICADA` → `DERIVADO_IDENTIFICADO` → `PUEDE_CONTENER_DECLARADO` → `POSIBLE_REACTIVIDAD_CRUZADA` → `REQUIERE_REVISION`. Esta precedencia no representa gravedad médica ni probabilidad de reacción. Las exenciones regulatorias permanecen en un canal separado y nunca suprimen observaciones de seguridad. Ver `docs/ingredient-library/38_ARCHITECTURAL_DECISION_RECIPE_SAFETY_AGGREGATION.md`.
