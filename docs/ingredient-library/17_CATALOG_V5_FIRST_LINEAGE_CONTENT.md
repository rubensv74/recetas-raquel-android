...
`IngredientCatalogAssetReader.DEFAULT_VERSION_DIRECTORY` now points to:
...
The instrumented import test now verifies:
...
Before expanding v5 lineage concepts further, run:
...
Expected Room schema history remains exactly:
...
No Room v4 schema should be generated by catalog v5.

## Next content after green gate

After v5 validates, continue with a separately reviewed derivative/form batch. Priority candidates are flours and single-source oils, but each clinically relevant derivative must pass the food-safety evidence gate independently of its lineage relation.
